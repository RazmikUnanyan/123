...
export default function VINScanner() {
  ...
  const startScan = async () => {
    ...
    try {
      ...
      // Попытка получить основную камеру через facingMode: { exact: 'environment' }
      ...
      try {
        ...
        // fallback если exact не поддерживается или отказано
        stream = await navigator.mediaDevices.getUserMedia({
      ...
      // Считаем, что основная камера — первая с facingMode environment
      // Или берем первую из списка (fallback)
      const environmentDevices = devices.filter((d) =>
          d.label.toLowerCase().includes("back") || d.label.toLowerCase().includes("rear")
      );
      const videoInputDeviceId = environmentDevices[0]?.deviceId || devices[0]?.deviceId;
        ...
        {isMobile ? (
            <>
              ...
              <p style={{ marginTop: 8, color: "#555" }}>
                Position the VIN inside the green frame for scanning
              </p>
            </>
        ) : (
            <div>
